...
if(isset($_POST['submit'])){
   // User ඇතුළත් කරන නම 'username' ලෙස database එකට යනවා
   $username = mysqli_real_escape_string($conn, $_POST['name']);
   $email = mysqli_real_escape_string($conn, $_POST['email']);
   $pass = mysqli_real_escape_string($conn, md5($_POST['password']));
   $user_type = 'user'; // Database එකේ user_type column එකට අගය යනවා

   // Validation: නමට අකුරු පමණක් තිබේදැයි පරීක්ෂා කිරීම
   if (!preg_match("/^[a-zA-Z\s]*$/", $username)) {
      ...
      if(mysqli_num_rows($select_users) > 0){
         ...
         // Database columns: username, email, password, user_type
         $insert_query = "INSERT INTO `users`(username, email, password, user_type) VALUES('$username', '$email', '$pass', '$user_type')";
...
<div class="container d-flex justify-content-center align-items-center" style="min-height: 100vh;">
   <div class="card shadow p-4" style="width: 400px; border-radius: 15px;">
      ...
      <?php
      if(isset($message)){
         foreach($message as $msg){ echo '<div class="alert alert-danger py-2 small">'.$msg.'</div>'; }
      }
      ?>
